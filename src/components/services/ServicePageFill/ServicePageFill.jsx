                        ...
                        {auth? (
                            ...
                            {myServices.some(item => item.service===title)? (
                                <div>
                                    ...
                                    {deleted?<div className="service-btn" onClick={()=>{addUserService({id: uuid(), service: title}); 
                            alert('You have successfully booked a service');
                            this.setState({deleted: false})}}>Booking</div>:(
                                        <span className="canc-s">You are booked<div className="canc-btn"
                                            onClick={()=>{deleteUserService(title);{this.setState({deleted: true})}}}
                                            >Cancel service</div></span>
                                    )}
                                
                                </div>
                                
                            ):<div className="service-btn" onClick={()=>{addUserService({id: uuid(), service: title}); 
                            alert('You have successfully booked a service');
                            this.setState({showBooking: this.state.showBooking})}}>Booking</div>
                            }
                            </div>
